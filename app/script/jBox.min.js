...
function jBox(type, options) {
	...
	this.options = {
		...
		confirmButton: '确认',	// Text for the submit button
		...
		autoClose: 1000,			// Time when jBox should close automatically
	...
	this.defaultOptions = {
		...
		'Alert': {
			target: jQuery(window),
			fixed: true,
			attach: jQuery('[data-alert]'),
			getContent: 'data-alert',
			content: 'Do you really want to do this?',
			minWidth: 320,
			maxWidth: 460,
			cancelButton: '确认',
		...
		'Notice': {
			...
			blockScroll: true,
			closeOnEsc: true,
			attach: jQuery('[data-notice]'),
			getContent: 'data-notice',
			closeOnClick: 'overlay',
			closeButton: true,
			overlay: true,
			animation: 'zoomOut',
	...
	this._create = function() {
		...
		if (this.options.closeButton) {
			this.closeButton = jQuery('<div/>', {'class': 'jBox-closeButton jBox-noDrag'}).text('关闭').on('touchend click', function(ev) { this.isOpen && this.close({ignoreDelay: true}); }.bind(this));
